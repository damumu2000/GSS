...
@php($serviceAgreementParagraphs = collect(preg_split('/\r\n|\r|\n/', (string) ($serviceAgreementContent ?? ''), -1, PREG_SPLIT_NO_EMPTY) ?: [])->map(fn ($item) => trim((string) $item))->filter()->values())
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="UTF-8">
...
<body data-login-messages='@json(array_values(array_filter(array_merge(
        ...
        <section class="auth">
            <div class="auth-inner">
                    ...
                    <form method="POST" action="{{ route('login.store', $loginFormSiteKey ? ['site' => $loginFormSiteKey] : []) }}" novalidate data-login-form>
                        ...
                        <div class="field">
                            ...
                            <div class="input-wrap">
                                ...
                                <input id="username" type="text" name="username" placeholder="请输入您的账号" value="{{ old('username') }}" autocomplete="username" required autofocus>
                            </div>
                        </div>
                        ...
                        <div class="row login-options">
                            ...
                            <label class="remember remember-agreement">
                                <input type="hidden" name="service_agreement" value="0">
                                <input type="checkbox" name="service_agreement" value="1" @checked((string) old('service_agreement', '1') === '1')>
                                <span>我已阅读并同意 <a class="link" href="#" data-open-service-agreement>服务协议</a></span>
                            </label>
                        </div>
                        ...
                        <div class="alt-grid">
                            <button class="alt-button" type="button" data-open-wechat-help>
                ...
                <div class="auth-footer">
                    <p>还没有账号？ <a class="link" href="https://www.guanshanshan.cn/cat/start" target="_blank" rel="noopener noreferrer">申请入驻</a></p>
                    <div class="footer-links">
                        <a href="https://www.guanshanshan.cn/cat/Privacy-Policy">隐私政策</a>
                        <span>|</span>
                        <a href="https://www.guanshanshan.cn/cat/Contact-Help">帮助中心</a>
                    </div>
                </div>
            </div>
        </section>
    ...
    <div class="confirm-modal" id="password-help-modal" aria-hidden="true">
        ...
        <div class="confirm-modal-dialog" role="dialog" aria-modal="true" aria-labelledby="password-help-title">
            ...
            <div class="confirm-modal-copy" id="password-help-copy">
                <p class="confirm-modal-note" data-password-help-default>
    ...
    <template id="service-agreement-template">
        @forelse ($serviceAgreementParagraphs as $paragraph)
