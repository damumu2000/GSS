...
@push('styles')
    <style>
        ...
        .role-choice.is-active .role-choice-check {
            background: var(--primary);
        ...
        .role-choice.is-active .role-choice-check::after {
            opacity: 1;
        }
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const syncStatusIdentity = function () {
                ...
                const seed = nameValue || usernameValue || '新';

                displayName.textContent = nameValue || '待填写姓名';
                displayUsername.textContent = usernameValue || '待设置账号';
                displayAvatar.textContent = Array.from(seed)[0] || '新';
            };
            ...
            const form = document.getElementById('site-user-create-form');
            const fields = {
            ...
            const validators = {
                ...
                password(value) {
                    ...
                    return value.length >= 8 ? '' : '初始密码至少需要 8 位。';
                },
                remark(value) {
...
@section('content')
    ...
    <section class="site-user-shell">
        <form id="site-user-create-form" method="POST" action="{{ route('admin.site-users.store') }}">
            @csrf
                                    ...
                                    <div class="site-user-status-hero">
                                        ...
                                        <div class="site-user-status-avatar-card" data-avatar-trigger>
                                            <div class="site-user-status-avatar" data-avatar-preview>
                                                <span class="site-user-status-avatar-fallback" data-status-avatar>{{ $draftAvatarInitial }}</span>
                                            </div>
                                        ...
                                        <div class="site-user-status-copy">
                                            ...
                                            <div class="site-user-status-copy-name" data-status-display-name>{{ $draftDisplayName }}</div>
                                            <div class="site-user-status-copy-subline">
                                                <span>后台操作员账号</span>
                                                <code data-status-display-username>{{ $draftDisplayUsername }}</code>
                                            </div>
